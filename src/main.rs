...
use geocode_mcp::server::McpServer;
use geocode_mcp::transport::StdioTransportHandler;
...
#[derive(Parser)]
#[command(name = "geocode-mcp")]
#[command(about = "Geocoding MCP Server")]
#[command(
    long_about = "geocode-mcp resolves location names to geographic coordinates as an MCP server for LLM orchestrators.\n\nUsage:\n  geocode-mcp serve --mode stdio\n  geocode-mcp serve --mode websocket --host 0.0.0.0 --port 8080"
)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
